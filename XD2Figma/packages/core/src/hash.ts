...
/** Pure JS SHA-256, usable in the restricted Figma plugin sandbox. */
export function sha256HexSync(data: Uint8Array | string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + upper1 + choice + K[index] + schedule[index]) >>> 0;
...
export function stableStringify(value: unknown): string {
  const result = serializeJson(value, false);
  if (result === undefined) throw new TypeError('The root value is not JSON-serializable.');
  return result;
}

function serializeJson(value: unknown, inArray: boolean): string | undefined {
  ...
  if (Array.isArray(value)) return `[${value.map((item) => serializeJson(item, true) ?? 'null').join(',')}]`;
  const object = value as Record<string, unknown>;
  const entries: string[] = [];
  for (const key of Object.keys(object).sort()) {
    const serialized = serializeJson(object[key], false);
